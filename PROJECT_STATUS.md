...
**Current turn:** RP-TURN-019-R1 is Decision required after its one authorized host rehearsal stopped at the orchestration output-capture boundary before service startup; the local WinSW drain implementation and repository gates are complete, but direct Stop-Service and crash-recovery proof remain unrun; Draft PR #17 remains open, Draft and unmerged
...
- RP-TURN-019-R1 retains WinSW 2.12.0 and adds an ACL-restricted local state-file lifecycle plus WinSW `stopexecutable`/`stoparguments`; this implementation is pending host verification and is not production approval
...
**RP-TURN-019-R1 — WinSW Local Drain and Graceful-Stop Correction (Decision required)**

RP-TURN-019 started from exact main `cd45e7356e902afbf3aafec0bdf8286dbccff7ad` on `agent/windows-vps-infrastructure-readiness`. The original run passed pinned-tool provenance, virtual-account services, immutable releases, loopback proxy/TLS/limits/streaming, independent restart, canary boundaries, forward/automatic/manual rollback and local-certificate reissue, then failed closed because default WinSW stop cut the synchronized stream. Project Codex authorized R1 to retain WinSW 2.12.0 and add one local-only drain. Implementation commit `ce13499ac4f879603eb8f1214b4a7129fba5004c` adds the protected `Ready → Draining → Stopped` state, fixed new-work 503, in-flight tracking, 15-second drain deadline, custom standalone launcher and WinSW stop hook. All repository, browser, database, audit, publication, client-boundary and four-scope Gitleaks gates passed. The one authorized live sequence installed the exact reviewed WinSW XML and protected control directory, verified the prior release manifest, then stopped before service startup because the external elevated output-capture wrapper promoted Caddy's informational stderr to a terminating `NativeCommandError`. It therefore produced no direct Stop-Service, stream-preservation or crash-recovery result and cannot support a reboot request. Cleanup left both services Stopped/Disabled, zero approved-port listeners, root processes, firewall rules, staging/rehearsal children and canary; no drain state remains. Draft PR #17 remains open, Draft and unmerged; `main` is unchanged. Another host attempt requires Project Codex authorization. RP-TURN-020 is unstarted and unauthorized.
...
| 019 | Decision required | R1 retains WinSW 2.12.0 and implements a protected local drain/stop hook with complete repository gates, but the single authorized live sequence stopped before service startup when its external output-capture wrapper treated Caddy informational stderr as terminating; direct Stop-Service, stream and crash gates remain unverified, final services are Stopped/Disabled with zero residue, and neither reboot nor public/production work was authorized |
