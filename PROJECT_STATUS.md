...
**Current turn:** RP-TURN-019-R4-DIAG4 future non-mutating elevation-probe mode is implemented pending Project Codex review; RP-TURN-019 remains Partial, PR #17 remains Draft/unmerged and no UAC, probe, Live rehearsal, reboot or deployment is authorized
...
- RP-TURN-019 accepted recovery state: both Rise Pals services are Stopped/Disabled at PID 0; original stalled processes, relevant listeners, enabled firewall rules, drain state, canary and raw-capture residue are absent; forced termination was recovery, not graceful-stop success, and no reboot/public deployment occurred
- RP-TURN-019 D-027 selects Option B for a repository-only prototype: a minimal self-contained .NET 10 LTS service-aware host preserving direct SCM Stop; this is not production acceptance or host authorization, and Option D remains unauthorized
...
- Final Windows Node supervisor selection, service-host signing/provenance, stop/preshutdown deadlines, release switching and monitoring implementation
...
**RP-TURN-019-R4 — Candidate Service Rehearsal Harness (repository-only; pending Project Codex review)**
...
Project Codex/Jeff selected Option B only for this repository prototype. Accepted R3-R1 evidence keeps the pinned portable .NET 10.0.400/net10.0-windows/win-x64 solution and locked Microsoft-only packages, and corrects three reviewed boundaries: exact candidate identity `RisePalsServiceHostCandidate` is shared by dispatcher and handler while retained service names are rejected; every failed create/assign/resume/Ready attempt is locally disposed, any possibly assigned Job is terminated and proven empty before terminal Stopped; and production evidence accepts only fixed typed events, bounded metadata and deterministic rotation while discarding raw Node stdout/stderr text. Fifty-one .NET tests passed, including six Node-fixture tests and fifteen focused correction regressions, and two clean self-contained single-file publishes were byte-identical. The executable remains NotSigned, so production installation is prohibited.

R4-R1 adds a deterministic repository-only harness around that accepted prototype. The corrected contract now pins its 73,606,961-byte executable SHA-256 `d86c4e4afcc8c1f6d8e77694b5de163185326c460fea1be50e5533d29aca0e8c`, schema and dependency manifest; the exact signed Node v24.18.1 executable; and the complete retained `RisePalsApp`/`RisePalsProxy` definitions, commands, accounts, Own Process types, executable bytes and Stopped/Disabled/PID 0 states. It derives exact candidate SID `S-1-5-80-146351416-2890358921-3199710220-422177557-4020786491`; separates nonce-scoped immutable staging from configuration/log/evidence; permits only SYSTEM, Administrators and the candidate virtual account with ReadAndExecute/Modify as appropriate; and encodes 23 exact rehearsal/failure/cleanup stages. Non-reboot Preshutdown evidence is limited to a read-only exact-timeout/accepted-controls registration query plus accepted repository handler/orchestration tests; actual system-delivered Preshutdown remains pending a separately authorized controlled reboot. Proxy independence is snapshot-based only: the retained proxy is never enabled, started or restarted, its complete definition must remain byte-for-byte equivalent, and direct candidate probes use loopback without a proxy dependency. The parent never captures native output, the child keeps stdout/stderr separate, and success requires explicit exit code plus a fresh nonce/head/script-hash/digest-bound single-use structured result with raw output removed. WhatIf/plan and deterministic tests do not reach live/elevated commands. No UAC, service, registry, listener, `C:\RisePals` path, firewall, certificate, reboot or deployment mutation occurred. See [Windows service-host prototype](infra/windows-service-host/README.md) and [Windows Service Supervision Decision Pack](docs/14_WINDOWS_SERVICE_SUPERVISION_DECISION.md).

LIVE1 stopped fail-closed before UAC during its fresh Phase A build because `ChildIsAssignedToJobBeforeSuspendedPrimaryThreadResumes` immediately queried Job accounting after child completion and passed 50/51 tests. R4-R2 keeps the immediate non-empty assertion before resume and all production code unchanged, but routes only post-exit/post-termination emptiness checks through a test-only monotonic 20 ms poll with a strict three-second maximum and one final deadline query. Native query exceptions still propagate; there is no skip, unbounded retry or unconditional success. The exact failed test passed 10/10 consecutive repetitions, the complete 51-test suite passed three consecutive runs and the focused repository infrastructure suite passed 2 files / 26 tests. A new live authorization is required after this bounded correction is reviewed.

LIVE2 also stopped before UAC and its authorization is invalid. R4-R3 corrects the old build-context-sensitive artifact pin by setting explicit service-host-only assembly identity and excluding discovered Git revision from InformationalVersion. Independent clean publishes from the Git-aware repository and two file-only copies at distinct paths and timestamps produced the same two-file inventory, lengths and SHA-256 values; both the old R3-R1 and current outer commit IDs are absent from executable bytes. The post-policy service-host production source tree is `125eb5a7765c58cbc7cee094fbe82207642fd2a5`. No UAC, Live mode or host mutation occurred.

LIVE3 later returned exit code 1 without a valid structured result. Recovery2 is Accepted and the host is back at zero residue, but the functional rehearsal is not accepted. The exact historic cause remains unknowable. R4-DIAG1-R2 preserves the truthful bootstrap/child marker ownership from R1 and replaces the premature single result with two authenticated durable records: an atomic, reopened pre-cleanup checkpoint and a separate schema-v3 authoritative result written only after cleanup is attempted. The final receipt binds the checkpoint digest to exact launch/marker/final state, cleanup attempted/completed state, invocation absence and sanitized residue counts/paths; it can report success only after both records independently persist and validate and no transient object remains. A clean-commit-only suite launches the committed parent through hidden Windows PowerShell 5.1 as ten separate non-elevated processes and treats only exit code plus independently reopened records as authority, never stdout. No UAC, Live mode, service control or `C:\RisePals` write occurred; another host attempt requires separate exact-head authorization.

LIVE4B ran exactly once at head `2beed00217eaed25f1157aa7499f9196e5e52e66` and remains Partial: the parent exited 86, the elevated child exited 1 and the validated child status was failure. Its durable checkpoint digest was `8be6008c7c82113a8728f14e30af3085d9378c70ddb1919a2532ab54d0f392b1`; its authoritative-result digest was `cc15d958d953cc67957b3a498bd26c3d26d6899e875dfd328e8874d92a286157`. Cleanup was attempted and completed, the invocation directory was absent, residue was zero and the final host state was safe. The exact internal failed stage and individual functional-gate outcomes are unknown and are not inferred. R4-DIAG2 improves only future evidence: a validated child result is reduced to a fixed stage/gate/lifecycle allowlist with its own digest, persisted in the reopened pre-cleanup checkpoint and carried identically into the post-cleanup result. It does not retroactively determine LIVE4B's cause and authorizes no additional Live run.

LIVE5 later stopped before elevated process creation at head `c73628aa5e9b6558ec9389a9a7b3429499b9ac07`: its parent classification was `elevated-process-launch-failure`, `processLaunched` was false and no bootstrap, child, Live gate or host mutation occurred. Existing DIAG2 evidence proves complete cleanup, zero residue and the safe final state, but the exact launcher failure cause remains unknown. R4-DIAG3 improves future evidence only by binding a closed, digest-authenticated launch diagnostic into both durable records. It records controlled launch disposition/failure categories, bounded numeric exception evidence, fixed launcher facts and only an argument count plus canonical digest. It neither reads old captures nor retroactively infers LIVE5's cause.

R4-DIAG4 adds a future-only `ElevationProbe` mode that shares Live's canonical PowerShell `RunAs`, hidden-window, wait/pass-through, quoting, nonce, authorization and script-hash launch builder, but dispatches a dedicated child that can validate only authenticated entry, exact repository/script provenance and a closed Administrator/high-integrity token state before writing a digest-bound result and exiting. Its 21 Live gates are always `not_applicable`, its Live lifecycle booleans are always false and it cannot satisfy Live functional success. DIAG4 itself did not request UAC or execute the probe. A separately authorized future passing probe would prove only the elevation transport boundary—not service supervision, graceful shutdown, Preshutdown or production readiness. LIVE5's exact cause remains unknown and RP-TURN-019 remains Partial.
...
| 019 | R4-DIAG4 complete pending review; turn Partial | LIVE5 stopped before elevated process creation with valid zero-residue cleanup evidence, while its exact launcher cause remains unknown. DIAG3 preserves future sanitized launch evidence; DIAG4 adds a future-only non-mutating elevation probe sharing Live's launch boundary. DIAG4 did not execute UAC or the probe, the candidate remains unsigned/uninstalled, PR #17 remains Draft and no Live/probe attempt, reboot or deployment is authorized |
