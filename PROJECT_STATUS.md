...
**Current turn:** RP-TURN-019-R2 is a repository-only Decision Request: the live recovery and exact residue cleanup are Accepted, the current WinSW supervision path is rejected pending a new decision, Option B (a minimal service-aware self-contained .NET 10 LTS Windows host) is recommended, Option D (deployment-orchestrated blue/green Caddy drain) is the fallback only with an explicit acceptance-contract change, and Draft PR #17 remains Open, Draft and unmerged
...
- RP-TURN-019 current WinSW supervision is rejected pending a Project Codex/Jeff decision; Option B is recommended because it preserves the direct SCM stop gate, while Option D is fallback only if Project Codex deliberately changes that gate
...
**RP-TURN-019-R2 — Windows Service Supervision Decision Pack (Decision required)**
...
R2 compares WinSW redesign, a repository-owned service-aware Windows host, IIS/ARR/HttpPlatformHandler, deployment-orchestrated blue/green Caddy drain and the actively maintained Shawl supervisor against sixteen criteria. It recommends a deliberately small self-contained .NET 10 LTS host that owns the Node Job Object, private drain, bounded SCM Stop Pending checkpoints, finite restart and sanitized evidence while preserving direct `Stop-Service`. Blue/green is fallback only; used alone it preserves planned-release response draining but requires an explicit decision that bare `Stop-Service` no longer guarantees drain. No architecture is selected until Project Codex/Jeff decides. This documentation-only turn performs no host mutation and leaves RP-TURN-020 unstarted and unauthorized. See [Windows Service Supervision Decision Pack](docs/14_WINDOWS_SERVICE_SUPERVISION_DECISION.md).
...
| 019 | Partial — Decision required | Project Codex Accepted the live recovery and raw-capture cleanup, not graceful stop: both services are Stopped/Disabled, all original stalled PIDs/listeners/firewall/drain/canary/raw-capture residue are absent and no reboot/public deployment occurred. Current WinSW supervision is rejected pending decision. R2 recommends a minimal service-aware self-contained .NET 10 LTS host; blue/green Caddy drain is fallback only with an explicit direct-Stop-Service contract change. PR #17 remains Draft and no further host attempt is authorized |
