...
**Current turn:** RP-TURN-019-R4-DIAG1-R1 durable-parent-evidence and truthful child-start correction is implemented pending Project Codex review; PR #17 remains Draft/unmerged and no live/elevated rehearsal, reboot or deployment is authorized
...
LIVE3 later returned exit code 1 without a valid structured result. Recovery2 is Accepted and the host is back at zero residue, but the functional rehearsal is not accepted. The exact historic cause is unknowable because the old boundary wrote no marker before typed child parameter binding, elevated `%TEMP%`-relative directory resolution, helper loading, ACL work and other pre-result initialization. R4-DIAG1-R1 separates transient invocation data from one explicit caller-supplied durable evidence directory, validates its authorized containment/non-reparse/three-principal ACL contract, writes and independently reopens one nonce/provenance/digest-bound parent result before transient cleanup and leaves it reviewable after launcher exit. The bootstrap now records only `bootstrap-started` and `child-launch-attempted`; the child itself records `child-started` before `live-started`, with coherent ordering enforced. Thirty-one non-elevated classification scenarios plus durable-result replay, ACL, atomic interruption, export-failure and cleanup simulations cover the required fail-closed states. No UAC, Live mode, service control or `C:\RisePals` write occurred; another host attempt requires separate exact-head authorization.
...
| 019 | R4-DIAG1-R1 complete pending review | Accepted Recovery2 state remains unchanged. D-027 selected Option B only for a repository-owned prototype; R1 preserves an authenticated durable parent result outside the transient invocation tree, makes `child-started` child-owned, distinguishes launch-attempted from process-started and expands deterministic fail-closed transport coverage. It remains unsigned and uninstalled; PR #17 remains Draft, main/host remain unchanged and no live/elevated attempt or reboot is authorized |
