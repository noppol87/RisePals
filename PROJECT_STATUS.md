...
**Current turn:** RP-TURN-019-R4-DIAG1 repository-only elevated-bootstrap/result-transport hardening is complete pending Project Codex review after LIVE3 produced exit code 1 without parent-validatable evidence; PR #17 remains Draft/unmerged and no live/elevated rehearsal, reboot or deployment is authorized
...
LIVE3 later returned exit code 1 without a valid structured result. Recovery2 is Accepted and the host is back at zero residue, but the functional rehearsal is not accepted. The exact historic cause is unknowable because the old boundary wrote no marker before typed child parameter binding, elevated `%TEMP%`-relative directory resolution, helper loading, ACL work and other pre-result initialization. R4-DIAG1 corrects that entire unobservable class: the non-elevated parent creates and validates one explicit protected non-reparse result directory; a primitive-argument, self-contained PowerShell 5.1 bootstrap validates authorization/provenance pins and atomically writes bootstrap/child-dispatch markers; the child writes a live-started marker before native live work; and the parent validates sequence, schema, nonce, authorization ID, HEAD, launcher/bootstrap/transport/child hashes, UTC freshness, digest, replay and final/exit consistency before emitting one deterministic classification. Twenty-nine non-elevated scenarios cover the required UAC, launch, bootstrap, child, live, final, malformed, stale, replay, linked-path, atomic-interruption and cleanup states. No UAC, Live mode, service control or `C:\RisePals` write occurred in DIAG1; another host attempt requires separate exact-head authorization.
...
| 019 | R4-DIAG1 complete pending review | Accepted Recovery2 state remains unchanged. D-027 selected Option B only for a repository-owned prototype; DIAG1 adds an explicit parent-validated result root, minimal marker-first elevated bootstrap, child/live stage markers, deterministic parent classifications and 29 non-elevated transport simulations after LIVE3's exact pre-marker cause proved unknowable. It remains unsigned and uninstalled; PR #17 remains Draft, main/host remain unchanged and no live/elevated attempt or reboot is authorized |
