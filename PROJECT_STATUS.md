...
**Current turn:** RP-TURN-019-R1C is complete pending Project Codex review: a repository-owned PowerShell 5.1 parent/child launcher now separates elevated process control from native stdout/stderr capture, validates a nonce/head/freshness/digest-bound sanitized result and passes 14 non-elevated simulations; no UAC, host mutation or live rehearsal was authorized or performed, and Draft PR #17 remains open, Draft and unmerged
...
**RP-TURN-019-R1C — Versioned Elevated-Rehearsal Launcher (Complete pending Project Codex review)**

RP-TURN-019 started from exact main `cd45e7356e902afbf3aafec0bdf8286dbccff7ad` on `agent/windows-vps-infrastructure-readiness`. The accepted R1B ACL correction remains at `ed2b8046d10bf5be26fe3ce35c77705630691e2c`; its later live attempt was not rerun after external `*>&1` capture converted Caddy informational stderr into terminating `NativeCommandError`. R1C adds a repository-owned non-elevated parent, elevated child, structured-result helper, controlled native fixture, service-stop helper and PowerShell 5.1 simulation suite. The parent verifies exact branch/head/clean worktree, creates a fresh UUID nonce and exact temporary child directory, uses `Start-Process -Wait -PassThru` and adds `-Verb RunAs` only in future Live mode. It never redirects or parses the elevated child's console streams. The child protects its nonce directory, starts the exact PowerShell/native target with structured arguments, stores stdout and stderr separately, classifies by ExitCode, removes raw captures and atomically writes only a sanitized digest-bound result. The result contract validates schema, nonce, requested head, coherent fresh UTC timestamps, status/exit agreement, completion markers, cleanup/resource counts, privacy and canonical SHA-256. Missing, malformed, stale, replayed, partial, mismatched or inconsistent results fail closed. PowerShell 5.1 simulation passes all 14 required cases, focused launcher tests pass 4/4, focused drain/infrastructure tests pass 34/34, the complete suite passes 48 files/449 tests, the 27-route build/check and zero-vulnerability audits pass, publication/scoring digests remain unchanged and prohibited output-merge patterns are zero. This repository-only revision requested no UAC, performed no host mutation or live rehearsal and does not authorize another attempt or reboot. Draft PR #17 remains open, Draft and unmerged; `main` is unchanged. RP-TURN-020 is unstarted and unauthorized.
...
| 019 | R1C pending review | Repository-owned PowerShell 5.1 parent/child launcher separates elevation from native stream capture, accepts only validated nonce/head/freshness/digest-bound sanitized results and passes 14 non-elevated simulations plus all repository gates. No UAC, service/host mutation, live rehearsal or reboot occurred; PR #17 remains Draft and another live attempt requires a separate exact-head authorization |
