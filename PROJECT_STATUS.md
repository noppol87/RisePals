...
**Current turn:** RP-TURN-019-R1B-PHASE2D is authorized and in progress: the exact stale drain-state file was revalidated and removed without changing the parent ACL, and the canonical bitmask/atomic-state correction is under verification before one final authorized non-reboot rehearsal; Draft PR #17 remains open, Draft and unmerged
...
| 019 | In progress | R1B Phase 2D accepted the exact inherited child ACL as within the protected-parent boundary, revalidated and removed the stale stopped state without changing the parent, and authorizes a canonical numeric ACL/atomic-state correction plus exactly one final non-reboot rehearsal; reboot and public/production work remain unauthorized |
