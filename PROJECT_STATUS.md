...
R4 adds a deterministic repository-only harness around that exact accepted prototype. The contract pins its 73,606,931-byte executable SHA-256 `04e18bae3d0165118aa54676210a0425ee8a220cf33b9a6e17c29462093b985f`, schema and dependency manifest; derives exact candidate SID `S-1-5-80-146351416-2890358921-3199710220-422177557-4020786491`; separates nonce-scoped immutable staging from configuration/log/evidence; permits only SYSTEM, Administrators and the candidate virtual account with ReadAndExecute/Modify as appropriate; and encodes 23 exact rehearsal/failure/cleanup stages. The parent never captures native output, the child keeps stdout/stderr separate, and success requires explicit exit code plus a fresh nonce/head/script-hash/digest-bound single-use structured result with raw output removed. The synthetic fixture now exposes only loopback readiness/three-chunk/crash endpoints for a separately authorized run. WhatIf/plan and deterministic tests do not reach live/elevated commands. No UAC, service, registry, listener, `C:\RisePals` path, firewall, certificate, reboot or deployment mutation occurred. See [Windows service-host prototype](infra/windows-service-host/README.md) and [Windows Service Supervision Decision Pack](docs/14_WINDOWS_SERVICE_SUPERVISION_DECISION.md).
...
| 019 | R4 complete pending review | Accepted recovery state remains unchanged. D-027 selected Option B only for a repository-owned prototype; R3-R1 fixed identity/startup/evidence boundaries, and R4 adds a pinned nonce/head/script-hash-bound candidate plan, exact SID/ACL/path policy, separate-stream structured launcher, loopback synthetic fixture and 23 fail-closed rehearsal/cleanup stages. It remains unsigned and uninstalled; PR #17 remains Draft, main/host remain unchanged and no live/elevated attempt or reboot is authorized |
