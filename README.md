...
**Project status:** RP-TURN-019 Windows VPS Infrastructure Readiness authorized and in progress / loopback-only non-public rehearsal  
...
RP-TURN-019 is separately authorized to rehearse a non-public Windows host boundary. It selects Next.js standalone on pinned Node 24.18.1, standard Caddy 2.11.4 and stable WinSW 2.12.0 for the bounded rehearsal only. Health behavior is fixed and non-sensitive, releases come from an exact committed archive with a deterministic file manifest and every endpoint remains on loopback. The turn does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
