...
**Project status:** RP-TURN-019-R1 Decision required at the live rehearsal orchestration boundary; non-public host left Stopped/Disabled  
...
RP-TURN-019 separately rehearses a non-public Windows host boundary with Next.js standalone on pinned Node 24.18.1, standard Caddy 2.11.4 and stable WinSW 2.12.0. After the original default-stop limitation, authorized R1 retained WinSW and added an ACL-restricted local lifecycle, fixed drain rejection, in-flight request tracking and a WinSW stop hook. Repository gates pass at implementation commit `ce13499ac4f879603eb8f1214b4a7129fba5004c`. The one live R1 sequence installed the reviewed XML/control ACL but stopped before service startup because its external output-capture wrapper converted Caddy informational stderr into a terminating PowerShell error. Direct `Stop-Service`, streamed-response and crash-recovery gates therefore remain unverified and another live attempt requires Project Codex authorization. Both services are Stopped/Disabled; approved-port listeners, processes, staging/rehearsal children and canary are absent. The turn does not authorize reboot, DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
