...
**Project status:** RP-TURN-019-R4-NODE-DIAG2 repository-owned destination diagnostic is implemented pending Project Codex review; RP-TURN-019 remains Partial, the protected Node destination remains unknown and the candidate remains unsigned, uninstalled and not production-approved  
...
NODE1/DIAG1 also remains Partial: its temporary path validator failed before protected inspection, and initialized zero/false values in that failed record are not evidence about the destination. NODE-DIAG2 prepares a future-only read-only PowerShell 5.1 diagnostic with component/canonical ancestry validation, a closed deterministic official inventory whose file hash is authorization-bound, closed digest-bound evidence, truthful `null`/`not_reached` state and a sole-missing-`node.exe` repair predicate. Its 25 simulations run in separate hidden non-elevated PowerShell processes and touch only fresh temporary fixtures. The real `C:\RisePals\tools\node\24.18.1` state remains unknown; this repository turn used no UAC, LiveReadOnly execution, protected inspection, Node restoration or Live rehearsal.
