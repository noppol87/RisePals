...
**Project status:** RP-TURN-019 Windows VPS Infrastructure Readiness Partial / Decision required; non-public host left Stopped/Disabled  
...
RP-TURN-019 is separately authorized to rehearse a non-public Windows host boundary. It selects Next.js standalone on pinned Node 24.18.1, standard Caddy 2.11.4 and stable WinSW 2.12.0 for the bounded rehearsal only. Provenance, immutable releases, loopback proxy/TLS/limits/streaming, independent restart, canary isolation and forward/automatic/manual rollback passed. The synchronized pinned-Node probe then showed that WinSW 2.12.0 cut an active stream during bounded `Stop-Service`, so the turn is Partial / Decision required under its explicit supervision stop condition. No supervisor substitution or weaker test is authorized. Both services are Stopped/Disabled; approved-port listeners and the canary are absent. The turn does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
