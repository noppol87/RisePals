...
**Project status:** RP-TURN-019-R4 repository-only candidate rehearsal harness is implemented pending Project Codex review; no mutating/elevated path was run and the candidate remains unsigned, uninstalled and not production-approved  
...
RP-TURN-019-R4 adds only a repository-owned candidate rehearsal harness. It pins the accepted unsigned executable, schema and dependency manifest; derives the exact virtual-service-account SID; defines nonce-scoped staging/config/log paths and three-principal ACLs; models one Own Process/Manual candidate plus a 30-second Preshutdown timeout above the complete 15-second drain plus 5-second exit bound; validates fresh digest-bound single-use structured results; and encodes 23 fail-closed future rehearsal/cleanup stages. Its deterministic plan/tests are non-elevated and read-only with respect to services and `C:\RisePals`. The future live path is gated by a separate exact-head authorization and was not invoked. Existing `RisePalsApp` and `RisePalsProxy` remain retained Stopped/Disabled; no UAC, service, listener, filesystem, firewall, DNS, certificate, reboot or deployment mutation is authorized by R4. Option D is still not authorized. The turn does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See also the [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
