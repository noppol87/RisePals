...
**Project status:** RP-TURN-019-R4-NODE-DIAG2-R1 exact protected-boundary diagnostic correction is implemented pending Project Codex review; RP-TURN-019 remains Partial, the protected Node destination remains unknown and the candidate remains unsigned, uninstalled and not production-approved  
...
NODE1/DIAG1 also remains Partial: its temporary path validator failed before protected inspection, and initialized zero/false values in that failed record are not evidence about the destination. NODE-DIAG2-R1 prepares a future-only read-only PowerShell 5.1 diagnostic that explicitly probes `C:\RisePals`, `tools`, `node`, `24.18.1` and `node.exe` in order with `Get-Item`/`Get-Acl` error semantics. Its closed five-result evidence records object, canonical parent/volume, filesystem, owner, ACL/ACE, reparse and exact sanitized failure provenance without treating `Directory.Exists=false` as absence. Descendants stay `not_reached` after an unsafe ancestor, and the sole-missing-`node.exe` predicate requires four coherent inspected directories, an explicit executable not-found result and a discrepancy-free inventory comparison. All 45 simulations run in separate hidden non-elevated PowerShell processes against fresh temporary fixtures. The real `C:\RisePals\tools\node\24.18.1` state remains unknown; this repository turn used no UAC, LiveReadOnly execution, protected inspection, Node restoration or Live rehearsal.
