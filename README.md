...
**Project status:** RP-TURN-019-R2 Decision required; live recovery and residue cleanup are Accepted, current WinSW supervision is rejected pending selection, and the repository-only decision pack recommends a minimal service-aware Windows host with blue/green Caddy drain as the contract-changing fallback  
...
The [Windows Service Supervision Decision Pack](docs/14_WINDOWS_SERVICE_SUPERVISION_DECISION.md) recommends a minimal repository-owned self-contained .NET 10 LTS Windows service host that reports bounded SCM Stop Pending checkpoints, owns the pinned Node process tree, initiates the existing private drain and preserves the direct `Stop-Service` gate. Deployment-orchestrated blue/green Caddy drain is the fallback only if Project Codex deliberately changes that gate; it can preserve planned-release responses but cannot by itself guarantee manual-stop, crash or reboot drain. No option is selected, implemented or installed in R2. The turn does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See also the [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
