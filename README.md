...
**Project status:** RP-TURN-019-R1C complete pending review; versioned PowerShell 5.1 parent/child launcher and sanitized result contract pass repository-only simulations, with no UAC, host mutation or live rehearsal authorized  
...
RP-TURN-019 separately rehearses a non-public Windows host boundary with Next.js standalone on pinned Node 24.18.1, standard Caddy 2.11.4 and stable WinSW 2.12.0. The accepted R1B ACL correction remains unchanged at `ed2b8046d10bf5be26fe3ce35c77705630691e2c`; its later live attempt was not rerun after external output capture changed Caddy stderr semantics. R1C now owns that boundary in Git: a non-elevated parent validates the exact clean head and launches one child without capturing its console, while the child alone starts native work, separates stdout/stderr, uses ExitCode as authority, deletes raw captures and atomically writes a sanitized nonce/head/freshness/digest-bound result. PowerShell 5.1 simulation covers informational stderr success, native failure, dual streams, missing/malformed/partial/stale/mismatched evidence, exit disagreement, cleanup residue, privacy and replay prevention. The repository-only revision does not request UAC, start services, change `C:\RisePals` or authorize another rehearsal/reboot. The turn also does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
