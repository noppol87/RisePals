...
**Project status:** RP-TURN-019-R4-DIAG1 elevated-bootstrap/result-transport hardening is implemented pending Project Codex review after LIVE3 exited without parent-validatable evidence; the recovered host remains at zero residue and the candidate remains unsigned, uninstalled and not production-approved  
...
R4-DIAG1 closes LIVE3's unobservable elevated-startup class without claiming an unknowable historical root cause. The previous child could fail during typed binding, elevated `%TEMP%`-relative path resolution, helper loading, ACL work or pre-result initialization before any durable marker. The corrected parent pre-creates and validates one explicit non-reparse, protected result directory; a minimal primitive-argument PowerShell 5.1 bootstrap validates authorization/provenance pins and atomically records bootstrap and child-dispatch markers before the full child starts; the child records a live-started marker before native live work; and the parent validates marker sequence, schema, nonce, authorization ID, HEAD, all script hashes, UTC freshness, digest, replay and exit/final consistency. Twenty-nine non-elevated scenarios cover success and every required missing, malformed, stale, replayed, mismatched, linked, atomic-interruption and cleanup outcome. No UAC or Live mode was used; another attempt requires separate exact-head authorization.
