...
**Project status:** RP-TURN-019-R1B-PHASE2D Decision required; cleanup and the pushed ACL/atomic-state correction passed, but the single final rehearsal stopped at the Caddy-validation output-capture boundary before live service gates and was not rerun  
...
RP-TURN-019 separately rehearses a non-public Windows host boundary with Next.js standalone on pinned Node 24.18.1, standard Caddy 2.11.4 and stable WinSW 2.12.0. After the original default-stop limitation, authorized R1 retained WinSW and added an ACL-restricted local lifecycle, fixed drain rejection, in-flight request tracking and a WinSW stop hook. Project Codex accepted that the inherited child ACL remained within the exact protected-parent allowlist and identified the rejection as a checker false negative: Windows materializes the application rule as the exact numeric `Modify|Synchronize` mask `1245631`, not bare `Modify`. Under R1B Phase 2D, the exact stale `stopped` state file was revalidated and removed without changing its parent ACL, and the repository correction compares canonical SID/numeric bitmask records, rejects all extra/deny/explicit child rules and validates same-directory temporary and final state files around atomic rename. All repository gates passed and the correction was committed and pushed at `ed2b8046d10bf5be26fe3ce35c77705630691e2c`. The single final rehearsal passed service-identity preconditions and the drain-control update, then stopped before the remaining live gates because external diagnostic capture converted Caddy informational stderr into terminating `NativeCommandError`; it was not rerun. Sanitized cleanup evidence reports both services Stopped/Disabled and zero process, listener, firewall, staging, rehearsal, drain-state/temp/lock and canary residue. This is not acceptance or reboot authorization. The turn does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
