...
**Project status:** RP-TURN-019-R4-DIAG1-R1 durable-parent-evidence and truthful child-start correction is implemented pending Project Codex review; the recovered host remains at zero residue and the candidate remains unsigned, uninstalled and not production-approved  
...
R4-DIAG1-R1 closes the remaining review gaps without claiming an unknowable historical LIVE3 root cause. The caller supplies a durable evidence directory; Live mode restricts it beneath `C:\Users\Administrator\Documents\Codex`, while simulations use a task-scoped temporary root. The parent atomically writes and independently reopens a nonce/authorization/HEAD/four-script-hash/digest-bound result there before deleting any transient marker or raw capture. The bootstrap owns `bootstrap-started` and `child-launch-attempted`; only the child process can write `child-started`, followed by `live-started`. Thirty-one non-elevated classification scenarios plus durable-result replay, ACL, path, atomic-interruption, export-failure and cleanup checks cover the corrected boundary. No UAC or Live mode was used; another attempt requires separate exact-head authorization.
