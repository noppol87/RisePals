...
**Project status:** RP-TURN-019-R3 repository-owned .NET Windows service-host prototype is complete pending Project Codex review; it is unsigned, uninstalled and not production-approved  
...
The [Windows Service Supervision Decision Pack](docs/14_WINDOWS_SERVICE_SUPERVISION_DECISION.md) records D-027's selection of Option B for a repository-only prototype. The [prototype](infra/windows-service-host/README.md) is a minimal self-contained .NET 10 LTS Windows host with native SCM Stop/Preshutdown states, bounded checkpoints, an ACL-restricted private named pipe, suspended-before-Job-assignment Node launch, whole-tree cleanup, finite restart and sanitized evidence. Its 37 .NET tests and repeated byte-identical single-file publication pass. It is intentionally NotSigned and has not been installed or run as a service; production use and another host rehearsal remain unauthorized. Option D is still not authorized. The turn does not authorize DNS, public 80/443, public ACME, real users/data, production Clerk/PostgreSQL/secrets, external monitoring, CI or launch. See also the [Windows VPS Infrastructure Readiness Runbook](docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md).
