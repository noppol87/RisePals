...
|---|---|---|---|
| Native Node standalone + Caddy as Windows services | Direct fit with confirmed Windows host; Caddy documents Windows service and reverse-proxy/HTTPS operation; no container prerequisite | WinSW 2.12.0 has no Authenticode/checksum asset and every service/ACL/restart/certificate behavior must pass the bounded rehearsal | Selected only for RP-TURN-019 non-public rehearsal: Node 24.18.1, Caddy 2.11.4 standard build and WinSW 2.12.0 stable |
...
The complete bounded Windows contract, verified tool provenance and operating sequence are in `docs/13_WINDOWS_VPS_READINESS_RUNBOOK.md`. Selection for rehearsal is not production approval and does not silently select deployment authentication, public certificate automation or firewall exposure.
...
- RP-TURN-019 uses stable WinSW 2.12.0 for Node only, with `autoRefresh=false`, bounded roll-by-size logging and a finite restart sequence. Caddy uses SCM directly. Running `npm start` interactively or using Task Scheduler is not a supervision plan.
...
- RP-TURN-019 selects an NTFS junction at `C:\RisePals\current` for its bounded rehearsal and must prove atomic forward switch, failed-candidate rollback and manual rollback before acceptance.
...
- Product measurement requires a separate exact current `measurement-monitoring` grant. Only newly applied explicit persisted operations can produce one allowlisted activation or later-UTC-day meaningful-return event. Exact replay—including replay after grant or subject rotation—passive GET/page view/refresh/navigation/language switch and denied/conflicting/failed mutations produce none.
...
The internal application facade validates and context-hashes a raw mutation UUID before the provider-neutral server-only adapter boundary. Every PostgreSQL, disabled or injected adapter receives exactly schema version, controlled surface/operation/locale and a SHA-256 action digest—never the raw UUID or domain payload. The PostgreSQL adapter binds that digest to the internal owner and relies on global digest uniqueness, preserving mutation provenance across per-grant subject rotation. A server-only applied/replayed/non-applied envelope controls capture without entering public DTOs or client props. Unexpected action failures report only controlled occurrence fields and return a fixed domain result without rethrowing the original message, stack or cause. The three measurement tables force owner RLS, require current consent and grant the application only SELECT/INSERT. No result, score, recommendation, lesson/evidence payload, raw response, provider/email, free text or secret is stored in telemetry.

### RP-TURN-018 maintenance and recovery boundary

Owner export stays in a server-only module and uses exact owner predicates, fixed query shapes and a 500-row per-section bound. The canonical JSON maps all database relationships to local references before serialization, so internal owner/session/run/attempt/artifact UUIDs never enter the bundle. The same unchanged state serializes byte-identically and includes controlled Thai/English limitations.

Erasure is outside the application architecture. A dedicated `NOLOGIN NOINHERIT NOBYPASSRLS` privacy role owns only the two security-definer maintenance functions and no application table. It receives no runtime login, browser route or application execute grant. A disposable bootstrap may temporarily grant the migration owner membership only to transfer function ownership, then must revoke it. The functions require exact `app.current_user_id`, an internal owner UUID and opaque request UUID; table-level privileges, forced-RLS policies and delete guards constrain the fixed private-table dependency order. Public definition tables are outside its grants.

The recovery rehearsal uses the pinned local PostgreSQL 18.4 tools and three separately named child databases inside one disposable loopback cluster. Backup/export bytes exist only beneath a verified GUID child of `%TEMP%\risepals-alpha-recovery` and are deleted in `finally`. Production backup encryption, off-host placement, RTO/RPO, deletion-ledger reconciliation and staff access are not selected. A restore from before erasure can resurrect deleted rows and therefore remains a production stop condition.
