...
R4 adds a future-run harness without executing it: exact accepted executable/schema/manifest pins, deterministic candidate SID, nonce-scoped staging/config/log/evidence paths, three-principal ACL classes, Own Process/Manual/Preshutdown plan, separate-stream parent/child execution, fresh single-use structured results, a loopback synthetic fixture/probe and 23 exact behavioral/failure/cleanup stages. Repository tests prove plan determinism and fail-closed boundaries without Live mode, UAC, service control or `C:\RisePals` writes. Any candidate installation or live rehearsal requires a separate authorization naming the exact reviewed R4 head; RP-TURN-020 remains unauthorized.
