...
R4-R1 adds a future-run harness without executing it: exact accepted host/schema/manifest and signed Node executable pins; exact retained App/Proxy command, account, Own Process, executable byte and Stopped/Disabled/PID 0 definitions; deterministic candidate SID; nonce-scoped staging/config/log/evidence paths; three-principal ACL classes; separate-stream parent/child execution; fresh single-use structured results; a loopback synthetic fixture/probe; and 23 exact behavioral/failure/cleanup stages. A non-reboot run may query only the exact configured Preshutdown timeout and advertised accepted control while the candidate is Running; it cannot claim control delivery, whose proof requires a separately authorized reboot. Retained-proxy independence uses complete before/after snapshots and direct loopback while the proxy remains Stopped/Disabled/PID 0, never a proxy restart. Repository tests prove plan determinism and fail-closed boundaries without Live mode, UAC, service control or `C:\RisePals` writes. Any candidate installation or live rehearsal requires a separate authorization naming the exact reviewed R4-R1 head; RP-TURN-020 remains unauthorized.
