...
Implementation status: the bounded branch adds one third migration, only `assessment_sessions`/`assessment_responses`, exact version/consent anchoring, forced owner RLS, append-only raw-option revisions, idempotent mutation keys and a protected Thai/English start/save/resume/submit route. Deterministic code/database/browser gates pass. The final bounded real-provider smoke verified Thai sign-up/onboarding, stable account mapping, current consent/profile, start/save/correction/refresh, complete atomic/idempotent submission, immutable receipt, explicit profile logout, logged-out denial, same-identity re-authentication, safe return handling, privacy/database evidence and complete identity/build/database cleanup. Verification is complete pending Project Codex review, and RP-TURN-013 is not authorized or started.
