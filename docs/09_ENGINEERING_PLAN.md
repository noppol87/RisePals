...
Implementation status: R1 Decision required on `agent/windows-vps-infrastructure-readiness`. The approved candidate remains Next.js standalone on Node 24.18.1 behind standard Caddy 2.11.4, with stable WinSW 2.12.0 supervising Node and SCM supervising Caddy. Provenance, exact release inventories, loopback proxy/TLS/limits/streaming, independent restart, canary isolation, forward switch, failed-candidate automatic rollback, manual rollback and certificate reissue passed in the original bounded run; default WinSW stop then cut its synchronized stream. Project Codex authorized R1 to retain WinSW and add one ACL-restricted local drain. Commit `ce13499ac4f879603eb8f1214b4a7129fba5004c` implements the 15-second `Ready → Draining → Stopped` state, new-work 503, in-flight tracking, stale-state reconciliation, fail-closed readiness and WinSW stop hook, with complete deterministic repository gates. The one R1 host sequence installed the exact XML and control ACL but stopped before service startup because its external elevated output-capture wrapper promoted Caddy informational stderr to a terminating PowerShell error. Direct Stop-Service, in-flight completion, crash recovery and reboot therefore remain unrun. Cleanup left both services Stopped/Disabled with zero approved-port listeners/processes/firewall rules, zero staging/rehearsal children and no canary. No public firewall rule, DNS, public certificate, production resource, real user/data, CI or deployment was created.

Another bounded non-reboot host run requires Project Codex authorization. A controlled reboot remains prohibited until the direct stop, stream, crash and every other non-reboot gate pass. RP-TURN-020 remains unauthorized.
