...
The four migrations create exactly seventeen tables: the accepted nine-table baseline, `user_profiles`, `assessment_sessions`, `assessment_responses` and the five RP-TURN-013 derived tables. Version rows must be inserted as drafts; publication runs database validation; publication-to-retirement must change status only; retired rows cannot update or delete. Exact canonical 8+2 and sealed-parent validation applies to published and retired history. Competency/item/mapping triggers lock every relevant OLD and NEW parent in deterministic order, blocking moves out of or into a sealed parent and closing concurrent publication/mutation races. The second migration adds controlled `profile-v1` checks, forced profile RLS and the narrow Clerk identity resolution function. The third adds only owner-scoped raw assessment persistence, exact version/consent anchors, forced RLS, revision/idempotency constraints and submission immutability. The fourth adds exactly five forced-RLS, append-only derived-result tables with atomic result-shape/evidence enforcement; no migration seeds a product result.
...
- RP-TURN-012 owner-scoped raw synthetic assessment start/save/resume/submit is Accepted by Project Codex for synthetic alpha. Its deterministic, disposable-database and final bounded real-provider verification gates pass. RP-TURN-013 was separately authorized and is pending review.
- The repository contains a static public narrative, synthetic assessment-domain definitions/tests, a Thai/English six-scenario usability player, the separate fixed example result, one repository-local lesson prototype, bounded account/profile/consent and raw assessment persistence implementations, and a pending-review synthetic persisted-result prototype. It is not a validated real assessment and creates no real-user result, validated recommendation, published/externally validated learning content or production account system.
...
### Actual RP-TURN-013 verification before Project Codex review
...
| one bounded Clerk Development smoke | PARTIAL — the single authorized command stopped during disposable bootstrap before the Clerk smoke script, browser context or identity creation because `bootstrap-disposable-postgres.mjs` still expected 12 tables while the authorized fourth migration correctly produced 17 (`17 !== 12`). No rerun or speculative fix was made. |
| failed-smoke cleanup | PASS — zero PostgreSQL/smoke Node processes, no PostgreSQL service, no disposable test root and no isolated smoke build remained. No synthetic identity was created. Ignored `.env.local` remained untracked and unchanged. |

RP-TURN-013 is not Accepted. A separate Project Codex revision authorization is required before changing the stale smoke-bootstrap assertion or running another real-provider attempt. RP-TURN-014 was not started.
