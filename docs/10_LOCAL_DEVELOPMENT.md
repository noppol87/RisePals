...
**Turn:** RP-TURN-019  
**Status:** RP-TURN-019-R4-NODE-DIAG2 repository-owned destination diagnostic complete pending Project Codex review; NODE1/DIAG1 and RP-TURN-019 remain Partial, the protected Node destination remains unknown, the prototype remains unsigned/uninstalled and no real users/data, production database, CI or deployment exists  
**Checked:** 2026-08-28
...
### RP-TURN-019-R4-NODE-DIAG2 repository-owned destination diagnostic
...
NODE-DIAG2 introduces a PowerShell 5.1-compatible diagnostic with separate `Simulation` and future `LiveReadOnly` modes. Production path validation normalizes approved separators, rejects rooted/empty/dot/dot-dot/invalid components, resolves each component with `GetFullPath`, requires strict ordinal-ignore-case ancestry and separately rejects root/child reparse points. The closed inventory requires exact properties/counts, deterministic ordinal ordering, unique relative paths, file/directory types, nonnegative lengths, lowercase SHA-256 values and a canonical records digest. Its exact file SHA-256 is also an authorization/evidence binding. Closed evidence binds authorization, nonce, repository head and diagnostic hash, represents unavailable values as `null`/`not_reached`, persists atomically outside `C:\RisePals` and must reopen independently. `nodeExeOnlyRepairSafe=true` is valid only after a complete comparison proves `node.exe` is the sole missing object.

The focused harness executes 25 separate hidden Windows PowerShell 5.1 simulations: complete except Node, complete including Node, absent/empty version, additional missing object, unexpected file/directory, content/type mismatch, root/child reparse, ancestry and access failures, seven malformed-inventory cases and five malformed/tampered/inconsistent evidence cases. All fixtures and evidence are temporary and non-elevated. LiveReadOnly remains future-only and was not called; no UAC, protected inspection, Node restoration, service/process/ACL/firewall mutation or Live rehearsal occurred.
