...
**Turn:** RP-TURN-019  
**Status:** RP-TURN-019-R3 repository-owned .NET Windows service-host prototype complete pending Project Codex review; unsigned, uninstalled and repository-only; no real users/data, production database, CI or deployment  
**Checked:** 2026-08-28
...
### RP-TURN-019-R3 repository-owned service-host evidence
...
The prototype separates orchestration from SCM, clock, Node child, Job Object, private drain, configuration and sanitized evidence adapters. Native SCM handling accepts Stop, Shutdown and Preshutdown, reports bounded pending checkpoints and never reports Stopped while the owned Job Object is non-empty. Preshutdown is required rather than ordinary Shutdown alone; a later installation must set a reviewed per-service `SERVICE_CONFIG_PRESHUTDOWN_INFO` timeout, while global `ServicesPipeTimeout` remains forbidden. No live SCM API or service configuration was invoked here.

The host starts only an exact Node executable and exact release entrypoint with structured Windows argument quoting. Native `CREATE_SUSPENDED` keeps fixture/application code from running until the child is assigned to a kill-on-close Job Object, closing the descendant-escape race. Standard output/error are independently read, redacted and size-rotated. The v1 named pipe uses `PIPE_REJECT_REMOTE_CLIENTS` and a protected DACL for the exact service identity plus `BUILTIN\Administrators`; its only fields are version, type, nonce, state and active count. Malformed/version-mismatched/stale/replayed/disconnected peers fail closed.

Thirty-seven .NET tests cover graceful and timeout stop, duplicate stop, Preshutdown, checkpoint monotonicity, finite crash restart/terminal failure, stop during backoff, exact path rejection, command-line quoting, no shell/npm/PowerShell runtime chain, redaction, DACL shape, malformed/stale/replayed protocol, three-chunk stream completion with new-work rejection, startup failure, stdout/stderr separation, suspended-before-Job assignment and child/descendant cleanup. The actual required commands and final results are recorded in the Turn handoff.

Two clean self-contained publishes produced identical two-file inventories. `RisePals.ServiceHost.exe` is 73,602,835 bytes with SHA-256 `be1c7c9ccee0920663de7504bcc558b8d1c50204d846c9d4486c5a5ee95f0191`; `service-host-config.schema.json` is 1,672 bytes with SHA-256 `64cd256addecd8489228f3ecfa6658d43eef897681326ffcd3bfd53c832a2b32`. The executable is NotSigned. No binary/publish directory is committed, and unsigned host installation remains prohibited.

R3 does not authorize a host installation, replacement service, UAC, registry change, `C:\RisePals` access, listener, firewall/DNS/certificate mutation, public traffic, reboot, deployment or RP-TURN-020. The two existing services must remain Stopped/Disabled.
