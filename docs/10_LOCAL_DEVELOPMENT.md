...
The seventh migration adds only `measurement_subjects`, `product_events` and `error_occurrences`, producing twenty-six tables. All three force owner RLS and exact current measurement consent. The application has SELECT/INSERT only. Product events use `product-measurement-v1`, only `activation_completed` or `meaningful_return_completed`, only assessment/result/lesson-practice/private-evidence surfaces and exact matching operation codes. Error occurrences use `redacted-error-occurrence-v1` and only controlled correlation/operation/surface/locale/category/severity/retryability/time plus optional context-bound digest fields. Withdrawal stops capture; re-grant rotates the subject; direct malformed or cross-owner operations fail closed.

Server Actions invoke measurement only after a successful accepted persisted mutation and report only controlled unexpected failure context. Passive reads, denials, conflicts and validation failures create no success event. Exact replay does not duplicate one. A meaningful return requires another successful explicit action on a later UTC date under an injectable clock. Adapter or reporter failure is caught outside the authoritative mutation and therefore cannot change its result, timestamp, lifecycle, replay contract or access. No provider SDK, external request, browser identifier, cookie, storage, beacon, pixel, raw event payload or staff dashboard exists.

| Verification | Result |
|---|---|
| focused consent/contract/adapter/action tests | PASS — 5 files / 47 tests cover independent consent identity/copy, exact allowlists, digesting, redacted occurrence rejection, disabled/PostgreSQL adapters, failure isolation, successful-only action instrumentation and bilingual client-safe controls |
| `npm run check` | PASS — Prettier, zero-warning ESLint, strict application/database typechecks, 41 files / 379 tests and secret-free production build of 27 pages/routes |
...
| `npm run db:test:disposable` | PASS — PostgreSQL 18.4 applied 385 statements across 7 migrations and created exactly 26 tables; consent, replay, later-day return, withdrawal, rotation, owner/context, append-only, field allowlist and role boundaries passed; process/data/log/credentials removed |
...
| external/client/privacy boundary | PASS — only repository-local PostgreSQL/disabled server adapters exist; no external telemetry or client identifier/payload was introduced |
| production/full npm audits | PASS — both dependency graphs report 0 vulnerabilities |
