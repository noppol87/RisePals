...
`tsconfig.typecheck.json` isolates standalone and production-build compiler passes from `.next/dev`. Next.js may retain development route declarations there after `next dev` while a production build retains equivalent declarations under `.next/types`; loading both generated sets produces duplicate identifiers. The typecheck-specific config inherits every strict compiler option and includes production route declarations under `.next/types`. `next.config.ts` selects the standard `tsconfig.json` for the development server and the isolated config for production/type verification, so both workflows retain route validation without generated declaration conflicts.
...
| `npm run format:check` | PASS — all matched files use Prettier style |
| `npm run lint` / `npm run typecheck` | PASS — zero ESLint warnings; strict `tsc --noEmit` exit 0 |
