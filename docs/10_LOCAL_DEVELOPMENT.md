...
**Turn:** RP-TURN-012  
**Status:** Bounded persisted synthetic-assessment implementation complete with Partial real-provider verification pending Project Codex review; no real users, production database/service or deployment exists  
**Checked:** 2026-08-16
...
The three migrations create exactly twelve tables: the accepted nine-table baseline, `user_profiles`, `assessment_sessions` and `assessment_responses`. Version rows must be inserted as drafts; publication runs database validation; publication-to-retirement must change status only; retired rows cannot update or delete. Exact canonical 8+2 and sealed-parent validation applies to published and retired history. Competency/item/mapping triggers lock every relevant OLD and NEW parent in deterministic order, blocking moves out of or into a sealed parent and closing concurrent publication/mutation races. The second migration adds controlled `profile-v1` checks, forced profile RLS and the narrow Clerk identity resolution function. The third adds only owner-scoped raw assessment persistence, exact version/consent anchors, forced RLS, revision/idempotency constraints and submission immutability; it seeds no product definition.
...
| one authorized real Clerk Development smoke | PARTIAL — one synthetic identity completed persisted start, first-answer correction, refresh resume, six-answer submission/double-submit protection, immutable receipt and database counts (1 submitted session, 7 historical revisions, 6 active). The harness then timed out looking for the profile-only logout control on the receipt route, so logout denial, repeat-sign-in restore and final browser/privacy assertions did not complete. The harness now navigates to profile first but was not rerun because authorization allowed at most one attempt. |
| failed-smoke cleanup | PASS — cleanup code deleted the unique synthetic identity and re-queried it absent without logging an identifier; no cleanup failure occurred. Read-only checks found zero PostgreSQL processes/services, zero disposable child roots/files, no isolated smoke build and no tracked `.env.local`; temporary data/logs/credentials were removed. |

The smoke outcome makes RP-TURN-012 Partial, not acceptance-ready. A rerun requires a new Project Codex authorization. The deterministic application/database evidence remains valid, but it does not substitute for the missing logout/re-auth/final privacy segment.
...
- RP-TURN-012 implements owner-scoped raw synthetic assessment start/save/resume/submit pending Project Codex review. Its deterministic and disposable database gates pass, but the single real-provider smoke is Partial as recorded above; RP-TURN-013 is not authorized.
