...
**Last updated:** 2026-09-01; R4-NODE-DIAG2-R2 hermetic Windows PowerShell module-loading correction pending Project Codex review while RP-TURN-019 remains Partial
...
NODE-DIAG2-R2 prepares, but does not execute, a corrected future `LiveReadOnly` inspection. One shared helper explicitly probes the exact root, tools, node, version and executable boundaries with error-producing `Get-Item` and `Get-Acl` calls; `Directory.Exists=false` and `Test-Path=false` are never accepted as protected-path absence. Its five ordered closed results preserve object/canonical-parent-volume/filesystem, owner, ACL/ACE, reparse, ancestry and exact failed operation/category/native/HRESULT evidence, while descendants remain `not_reached` after an unsafe ancestor. Before any `Get-Acl`, the shared Windows PowerShell bootstrap requires version 5.1, validates the exact regular/non-reparse `$PSHOME\Modules\Microsoft.PowerShell.Security` manifest, imports that literal manifest and verifies loaded module/path/base plus command provenance. It never rewrites `PSModulePath` or a profile. Mixed PowerShell-7-first tests retain all 45 boundary simulations and the candidate harness; five missing/reparse/outside/mismatch workers fail closed with sanitized stage/category evidence and zero capture residue. Project Codex must accept the committed implementation and issue a new exact authorization before any UAC or protected inspection; Node restoration and Live rehearsal require still-separate permission.
