...
NODE-DIAG2-R3 prepares, but does not execute, a corrected future `LiveReadOnly` inspection. One shared helper explicitly probes the exact root, tools, node, version and executable boundaries with error-producing `Get-Item` and `Get-Acl` calls; `Directory.Exists=false` and `Test-Path=false` are never accepted as protected-path absence. Its five ordered closed results preserve object/canonical-parent-volume/filesystem, owner, ACL/ACE, reparse, ancestry and exact failed operation/category/native/HRESULT evidence, while descendants remain `not_reached` after an unsafe ancestor. Before any `Get-Acl`, the shared Windows PowerShell bootstrap requires version 5.1, validates the exact regular/non-reparse `$PSHOME\Modules\Microsoft.PowerShell.Security` manifest, imports that literal manifest into process-global scope and verifies loaded module/path/base plus command provenance. Process-global import keeps the command callable after the initializer returns but does not persist an environment or profile change. Real incompatible PowerShell-7-first tests retain exact post-return provenance, all 45 boundary simulations and the candidate harness; five missing/reparse/outside/mismatch workers fail closed with sanitized stage/category evidence and zero capture residue. Project Codex must accept the committed implementation and issue a new exact authorization before any UAC or protected inspection; Node restoration and Live rehearsal require still-separate permission.

NODE-DIAG4 prepares a separate future-only failure transport around that diagnostic. It creates and reopens a durable request before launch, then uses an early child marker contract that is loaded before the security bootstrap and schema-v2 diagnostic contract. Every marker is nonce/head/hash bound and chained to the request digest. After process exit, the parent validates marker order/bindings/replay and independently reopens schema-v2 when it exists; it treats exit and evidence as separate facts. An atomically persisted and reopened checkpoint precedes removal of the transient marker directory. A separate final result then records only closed failure/lifecycle facts, digests and zero-or-controlled residue. Raw streams, error text/type/stack, command arguments, environment values, ACL contents and secrets are never durable evidence. Sixteen isolated non-elevated simulations cover the full authorized success/failure/tamper/cleanup matrix. This does not infer NODE-DIAG3's unknown cause and is not permission to run `LiveReadOnly`; any future elevation requires a new exact-head authorization after review.

NODE-DIAG5 stopped before DIAG4 request persistence; it supplied no destination evidence and its cause remains unknown. NODE-DIAG6 moves a durable future boundary outward: a raw-argument parent entry writes its first nonce/head/hash-bound marker before importing its own contract, then validates the exact 13-stage/16-category chain through independent DIAG4 request reopen, parent checkpoint reopen and cleanup. The repository-only `PreflightOnly` mode exercises that chain without dispatching the inner transport or reaching UAC, an elevated child or any protected path. Forty-one isolated simulations cover each artifact digest and authorized failure/tamper/stale/partial/replay/cleanup case. This runbook does not authorize `LiveReadOnly` or another host attempt.
