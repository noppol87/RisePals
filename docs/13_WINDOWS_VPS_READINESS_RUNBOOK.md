...
The app can read Node/WinSW/current release/config and only the temporary rehearsal canary; it can modify only its app log and shared Next cache. The proxy can read Caddy/config and modify only proxy log/Caddy certificate state. The secret DACL contains SYSTEM, Administrators and the application service only. Neither service runs as Administrator, LocalSystem, NetworkService or Jeff's interactive account.
...
- health tooling verifies redirect, explicit-CA HTTPS, 413 oversized body, unbuffered streaming, admin reload and exact loopback listeners.
