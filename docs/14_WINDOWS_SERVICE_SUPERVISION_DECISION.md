...
**Turn:** RP-TURN-019-R4  
**Status:** Option B repository-only prototype retained; R4-R4-R1 exact test-only named-pipe readiness diagnostics complete pending Project Codex review while RP-TURN-019 remains Partial  
...
LIVE5 later stopped before the elevated process was created at head `c73628aa5e9b6558ec9389a9a7b3429499b9ac07`. Its classification, false process flag, unreached bootstrap/child/Live state and DIAG2 cleanup evidence prove a safe zero-residue outcome, not the exact launcher cause. R4-DIAG3 therefore adds future-only closed launch evidence without changing the service host or DIAG2 child contract. A digest-authenticated 17-field diagnostic binds controlled launch/failure state, bounded optional numeric exception evidence, approved launcher basename/existence/signature/verb, wait/hidden-window flags and only argument count plus canonical digest. The checkpoint/result pair must carry equivalent launch-diagnostic digests; cancelled/failed launches cannot claim later stages, and success requires a returned process object. Raw arguments, paths, exception text/types/stacks, streams and environment data remain prohibited. DIAG3 does not retroactively infer LIVE5's failure or authorize another UAC/Live attempt, host mutation, reboot or deployment.

R4-DIAG4 adds a distinct future-only `ElevationProbe` execution mode to test that unresolved boundary without exposing the host to the Live sequence if elevation succeeds. It reuses Live's canonical parent `RunAs` builder and authenticated bootstrap/marker transport, then dispatches a dedicated child limited to nonce/authorization/head/script provenance and closed Administrator/integrity evidence. The probe diagnostic is digest authenticated, records no identity/token/path/argument/environment/raw error data and requires both `liveSequenceInvoked` and `hostMutationAttempted` to remain false. All 21 Live gates are `not_applicable`, every Live lifecycle boolean is false and probe success cannot satisfy Live success. DIAG4 did not request UAC or execute the probe. A later separately authorized passing probe would prove only elevation transport—not supervision, drain, Stop/Preshutdown or graceful shutdown. LIVE5's exact cause remains unknown and RP-TURN-019 remains Partial.

NODE1/DIAG1 separately remains Partial. It failed in temporary relative-path validation before protected-boundary inspection, so initialized zero/false values are not measurements and no conclusion may be drawn about `C:\RisePals\tools\node\24.18.1`. NODE-DIAG2-R3 retains the five ordered explicit probes and the exact Windows PowerShell 5.1 security-module bootstrap shared by the Node diagnostic and candidate rehearsal paths. The bootstrap validates the canonical regular/non-reparse manifest below `$PSHOME\Modules`, imports only that manifest into process-global scope and verifies the loaded module, ModuleBase and `Get-Acl` provenance after the initializer returns, without persistently modifying `PSModulePath`. Real incompatible mixed-module-path verification passes the explicit post-return check, candidate harness, all 45 boundary scenarios and five closed bootstrap failures with removed captures. LiveReadOnly was not executed and no UAC, protected inspection, restoration, service/ACL mutation or Live rehearsal occurred. This does not change the accepted service-host behavior, signed tool pins or supervision decision.

NODE-DIAG4 adds only a future pre-evidence transport and leaves schema-v2 measurement unchanged. A fresh atomically written request binds nonce, authorization, head and exact launcher/bootstrap/child/diagnostic/inventory hashes before launch. The child imports the early transport first and emits authenticated hash-chained stage markers before security-module initialization, contract import, parameter validation, dispatch and schema-v2 persistence. The parent separately records process creation/exit, validates and reopens any schema-v2 evidence, atomically persists/reopens a pre-cleanup checkpoint and deletes transient markers only afterward. Its final reopened result fails closed on binding/order/replay/digest, cleanup and persistence problems and retains no raw stream, exception, argument, environment, ACL or secret material. Sixteen separate hidden Windows PowerShell 5.1 simulations cover the bounded matrix without retry. DIAG4 neither reconstructs NODE-DIAG3 nor authorizes UAC, protected inspection, Node restoration or Live rehearsal; RP-TURN-019 remains Partial.
...
R4-R4-R1 responds only to LIVE7's pre-UAC Phase A timeout. It leaves the production host, protocol and three-second test deadline unchanged and adds a closed schema-v1 fixture diagnostic sequence beneath one nonce-bound temporary directory. The parent still accepts readiness only from the genuine private named pipe. Exact required-once properties, typed nullable exit code, filename/sequence binding and nonnegative monotonic elapsed time reject missing, duplicate, unknown, mistyped, replayed or malformed records. Bounded atomic-publication observation preserves the primary timeout, exit or protocol classification and records diagnostic validity separately. Setup is observed under its own bound; flat cleanup rejects directories, reparse objects and unattributable residue before exact deletion. The 26-test diagnostics suite, 38/38 focused fixture/pipe/drain tests, 10/10 isolated affected-test runs and three consecutive 77/77 service-host suites pass. No UAC, candidate install or host mutation is authorized by this evidence.
