...
**Turn:** RP-TURN-019-R4  
**Status:** Option B repository-only prototype retained; R4-NODE-DIAG2-R1 exact protected-boundary diagnostic correction complete pending Project Codex review while RP-TURN-019 remains Partial and the protected Node destination remains unknown  
...
NODE1/DIAG1 separately remains Partial. It failed in temporary relative-path validation before protected-boundary inspection, so initialized zero/false values are not measurements and no conclusion may be drawn about `C:\RisePals\tools\node\24.18.1`. NODE-DIAG2-R1 replaces only the future diagnostic design: exactly five ordered explicit probes, closed per-boundary object/canonical-parent-volume/filesystem/owner/ACL/ACE/reparse facts, exact exception provenance, truthful descendant `not_reached` state, an authorization-bound inventory-file hash and a strict sole-missing-`node.exe` repair predicate. The same production exception mapper handles simulated and future LiveReadOnly access denial. All 45 verification scenarios use non-elevated temporary fixtures and include all five denial levels, owner/ACL split failure, Deny/unexpected ACEs, every directory reparse boundary, false-absence rejection and provenance/digest tampering. LiveReadOnly was not executed and no UAC, protected inspection, restoration, service/ACL mutation or Live rehearsal occurred. This does not change the accepted service-host behavior, signed tool pins or supervision decision.
