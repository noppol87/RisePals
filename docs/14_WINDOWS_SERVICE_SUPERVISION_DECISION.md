...
**Turn:** RP-TURN-019-R4  
**Status:** Option B repository-only prototype retained; R4-NODE-DIAG2-R2 hermetic Windows PowerShell module-loading correction complete pending Project Codex review while RP-TURN-019 remains Partial and the protected Node destination remains unknown  
...
NODE1/DIAG1 separately remains Partial. It failed in temporary relative-path validation before protected-boundary inspection, so initialized zero/false values are not measurements and no conclusion may be drawn about `C:\RisePals\tools\node\24.18.1`. NODE-DIAG2-R2 retains the five ordered explicit probes and adds an exact Windows PowerShell 5.1 security-module bootstrap shared by the Node diagnostic and candidate rehearsal paths. The bootstrap validates the canonical regular/non-reparse manifest below `$PSHOME\Modules`, imports only that manifest and verifies the loaded module, ModuleBase and `Get-Acl` provenance without modifying `PSModulePath`. Mixed-module-path verification passes the candidate harness, all 45 boundary scenarios and five closed bootstrap failures with removed captures. LiveReadOnly was not executed and no UAC, protected inspection, restoration, service/ACL mutation or Live rehearsal occurred. This does not change the accepted service-host behavior, signed tool pins or supervision decision.
