...
**Turn:** RP-TURN-019-R4  
**Status:** Option B repository-only prototype retained; R4-DIAG1-R1 durable-parent-evidence and truthful child-start correction complete pending Project Codex review after LIVE3's functional rehearsal remained unaccepted  
...
LIVE3 later returned exit code 1 with no valid result. Recovery2 is Accepted and the host is again zero-residue; LIVE3 is not functional evidence. The old child had no marker ahead of typed binding, elevated `%TEMP%`-relative path resolution, helper loading and ACL/result initialization, so the exact historical failure is unknowable. R4-DIAG1-R1 fixes the reviewable-evidence and start-semantics gaps without naming an unsupported cause: transient invocation data is separate from a caller-supplied protected durable evidence directory; the bootstrap writes only `bootstrap-started` and `child-launch-attempted`; the child owns `child-started` and then `live-started`; and the parent validates exact provenance/order/freshness/replay/digest/final agreement before atomically writing and independently reopening a schema-v2 parent result. Transient cleanup is permitted only after that validation. Thirty-one classification scenarios plus durable-result path/ACL/replay/atomic/export/cleanup simulations cover the required states. This changes result transport only, not the candidate service contract or behavior. No UAC or Live attempt occurred and another one requires separate exact-head authorization.
