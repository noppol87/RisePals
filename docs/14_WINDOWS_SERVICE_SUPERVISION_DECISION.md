...
**Turn:** RP-TURN-019-R4  
**Status:** Option B repository-only prototype retained; R4-NODE-DIAG2 repository-owned destination diagnostic complete pending Project Codex review while RP-TURN-019 remains Partial and the protected Node destination remains unknown  
...
NODE1/DIAG1 separately remains Partial. It failed in temporary relative-path validation before protected-boundary inspection, so initialized zero/false values are not measurements and no conclusion may be drawn about `C:\RisePals\tools\node\24.18.1`. NODE-DIAG2 replaces only the future diagnostic design: component/canonical ancestry validation, closed official/destination inventories, an authorization-bound inventory-file hash, digest-bound atomic evidence with truthful unavailable states and a strict sole-missing-`node.exe` repair predicate. All 25 verification scenarios use non-elevated temporary fixtures. LiveReadOnly was not executed and no UAC, protected inspection, restoration, service/ACL mutation or Live rehearsal occurred. This does not change the accepted service-host behavior, signed tool pins or supervision decision.
