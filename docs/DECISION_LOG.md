...
- **Consequence:** Event classes are only `activation_completed` and `meaningful_return_completed`; surfaces are only assessment, result, lesson practice and private evidence; operation/category/severity fields are exact allowlists. Passive reads and denied/conflicting/failed actions create no success event. Exact replay cannot duplicate an event. Raw UUIDs are context-hashed before event storage. Error rows cannot contain messages, stacks, URLs, identity/profile data, responses, scores, lesson/evidence content, arbitrary JSON or secrets. The normal application role has SELECT/INSERT only; direct constraints, current-consent forced RLS and append-only triggers fail closed. Adapter/reporter failure cannot change domain results, timestamps, replay or access. No external request, SDK, collector, cookie, beacon, browser storage, dashboard or production monitoring coverage is introduced.
