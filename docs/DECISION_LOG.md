...
- **Date:** 2026-08-23
- **Status:** Implemented for RP-TURN-016 pending Project Codex review; not yet Accepted
...
- **Consequence:** Drafts may be partial and save only controlled IDs. Exact request replay returns the existing revision; conflicting UUID reuse or stale/concurrent successors append nothing. Only the exact five-part canonical structure may move `draft → ready`; content is then immutable. Draft or ready may become permanently read-only `withdrawn`; no operation reopens or deletes it. Ready means only that a synthetic structural checklist passes. It changes no XP, assessment, score, multiplier, priority, stage, recommendation, employment or eligibility state. No free text, URL, upload/file/media, object storage, download/export, share token, employer/reviewer access, analytics or browser persistence exists.
