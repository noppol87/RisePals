...
- **Consequence:** Published 8+2 framework metadata, scoring-model and assessment definitions are immutable; the eight exact core weights total 10,000 basis points; multipliers remain unweighted; consent is append-only; unsafe foreign-key deletion fails closed; JSON content requires an explicit schema version. Application credentials cannot migrate schema or bypass RLS, and browser code must never receive a database credential or control the trusted context directly.
