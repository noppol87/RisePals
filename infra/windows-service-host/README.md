...
The executable project alone pins Version/InformationalVersion `0.1.0-rp19-prototype`, AssemblyVersion/FileVersion `0.1.0.0` and disables source-revision suffixing. The manifest records service-host source tree `775a10c75fb70aa121350707ee1240c9d1739fa2` after the schema-aligned camel-case JSON reader correction; volatile outer-repository commit metadata is excluded from artifact identity. On 2026-09-05, three clean publishes—one Git-aware and two file-only at distinct paths and timestamps—produced the exact same two output files. The executable pin is 73,606,961 bytes, SHA-256 `f353a61f5a91751602dc37f7dbe6b0dbad9ab4ce5a59f84d8a3ff1cfd70a27bb`, NotSigned. Case-insensitive property matching remains disabled and unknown JSON properties remain rejected; the published camel-case schema is unchanged. The four focused JSON cases and all 81 service-host tests passed, including three consecutive full-suite runs. This remains an unsigned candidate, not production approval.
