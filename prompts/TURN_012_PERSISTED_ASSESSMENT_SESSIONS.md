...
Project Codex Accepted RP-TURN-012 for synthetic alpha. Implementation and deterministic/database/browser gates completed. The final bounded real-provider smoke used one new synthetic identity and verified Thai email-code sign-up/onboarding, one stable internal account mapping, current consent/profile persistence, persisted start/save/correction/refresh, complete atomic/idempotent submission, immutable receipt reload, explicit profile logout, logged-out same-locale denial, same-identity re-authentication, safe return handling and final browser/database privacy boundaries. Cleanup deleted and re-queried the identity absent and removed the isolated build plus disposable PostgreSQL process/data/logs/credentials. This acceptance does not authorize real users, production use or RP-TURN-013; RP-TURN-013 remains recommended but unstarted and unauthorized.
