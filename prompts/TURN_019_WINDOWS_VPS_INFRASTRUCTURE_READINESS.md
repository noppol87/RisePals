...
The corrected non-elevated parent pre-creates and validates one explicit non-reparse result root and nonce directory with only the current account, SYSTEM and Administrators. A self-contained PowerShell 5.1 bootstrap accepts primitive strings only, validates authorization ID, nonce, expected HEAD and committed launcher/bootstrap/transport/child SHA-256 values using `SHA256.Create().ComputeHash(...)`, and atomically writes `bootstrap-started` before full-child dispatch. It writes `child-started` before starting the committed child without stdout/stderr redirection; the child writes `live-started` before native live work. A top-level fixed bootstrap-failure marker and deterministic parent classifications must distinguish UAC/launch, never-entered, bootstrap-only, child-pre-live, live-without-final, valid-final and invalid/inconsistent evidence.
