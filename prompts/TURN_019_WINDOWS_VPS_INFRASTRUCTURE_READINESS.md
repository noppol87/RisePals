...
## RP-TURN-019-R4-DIAG1-R2 amendment — post-cleanup authority and process exit

The durable protocol has two independent atomic records. `candidate-parent-checkpoint-<nonce>.json` is written, reopened and validated before transient cleanup and contains the authenticated pre-cleanup classification plus its canonical digest. `candidate-parent-result-<nonce>.json` is written only after cleanup is attempted and binds the checkpoint filename/digest to launch/marker/final state, cleanup attempted/completed state, invocation absence, sanitized residue counts/paths, UTC and a separate canonical digest. Success requires a valid successful child final, both validated durable records and zero transient or temporary residue. Cleanup failure persists failure without speculative deletion; authoritative-result persistence failure exits nonzero while preserving the checkpoint. Both records use CreateNew/temp/move, exact containment, no-link/ACL/replay gates and exclude raw streams, environment values, request data, secrets and personal data.

A clean-commit-only repository suite must launch the actual committed parent in `Simulation` through a hidden Windows PowerShell 5.1 process, wait for exit and independently reopen durable evidence without parsing stdout. Ten isolated cases cover success, child launch failure, exit before the child-owned start marker, child before live, live without final, invalid final, cleanup failure, interrupted checkpoint write, interrupted final-result write and a pre-existing durable path. It asserts unchanged services/listeners/`C:\RisePals` processes and removes only test-owned temporary evidence after validation. R2 changes no service-host behavior and authorizes no UAC, Live mode, host/process/service/`C:\RisePals` mutation, reboot, public change, merge or RP-TURN-020 work.

## RP-TURN-019-R4-DIAG2 amendment — durable sanitized child diagnosis

LIVE4B ran exactly once and failed. Its authenticated R2 records prove cleanup completed, the invocation directory is absent, transient/temporary residue is zero and the final host is safe. They do not prove an exact internal failed stage or individual functional-gate disposition, and DIAG2 must not infer or reconstruct either.

For future executions, the parent must validate the transient child result and derive a strict allowlisted diagnostic before cleanup. The diagnostic authenticates child-result digest/status, controlled failure stage/code, a contiguous ordered completed-stage prefix, exactly 21 fixed functional gates using only `passed`, `failed`, `not_reached` and `not_applicable`, child cleanup, cleanup-responsibility transfer and monotonic Live-mutation/service-install/service-start/direct-Stop booleans. Exact property sets, stage/gate vocabulary, order, failure correspondence, lifecycle implications, privacy and canonical digest are mandatory. Unknown, duplicate, reordered, missing, extra, inconsistent or tampered values fail closed.

The schema-v2 pre-cleanup checkpoint atomically persists, reopens and validates this diagnostic before any transient result deletion. The schema-v4 post-cleanup result carries the identical diagnostic and parent validation compares its digest with the reopened checkpoint. Live success additionally requires every functional gate to pass; durable persistence/reopen/equivalence and zero-residue cleanup remain mandatory. An interrupted final result preserves an earlier valid checkpoint and exits nonzero. Deterministic simulation and hidden Windows PowerShell 5.1 process tests cover the authorized failure boundaries without stdout authority. DIAG2 changes no service-host behavior and authorizes no UAC, Live execution, service/process/`C:\RisePals` mutation, reboot, deployment, merge or RP-TURN-020 work.

## RP-TURN-019-R4-DIAG3 amendment — durable elevated-launch diagnosis

LIVE5 stopped before elevated process creation at head `c73628aa5e9b6558ec9389a9a7b3429499b9ac07`. Its parent classification, false process flag and unreached bootstrap/child/Live gates prove no elevated or host mutation occurred; the DIAG2 records prove complete zero-residue cleanup and a safe host. They do not establish the exact launcher failure. DIAG3 must not read old captures or retroactively infer that cause.

For future runs, the parent creates a closed `rise-pals-candidate-launch-diagnostic-v1` record that binds launch-attempt/process-created booleans, exact disposition and failure vocabularies, optional integral native/HResult values, a maximum four-level exception depth, fixed approved launcher basename, pre-launch existence and normalized signature status, exact verb, argument count and canonical SHA-256 digest, required wait/hidden flags and a canonical diagnostic digest. Exception messages, stacks, arbitrary type names, raw arguments/command lines, arbitrary paths, streams, environment values and sensitive data are prohibited. Native code 1223 at any bounded exception level is `uac-cancelled`; only reviewed codes receive another controlled category, while unknown numeric codes use `launcher-exception-unknown`.

Checkpoint schema v3 validates the launch diagnostic before transient cleanup. Result schema v5 must preserve an equivalent diagnostic digest. Missing, extra, malformed, inconsistent, tampered or non-equivalent launch evidence fails closed. Cancellation or launch failure cannot claim bootstrap, child or Live stages. Live success additionally requires disposition `launched` and a process object actually returned by `Start-Process`; all DIAG2 child stage/gate/lifecycle and cleanup success requirements remain unchanged. Repository-only PowerShell 5.1 tests cover successful creation, outer/inner cancellation, reviewed/unknown/no-native failures, malformed construction, property/state/digest tampering, checkpoint/result mismatch and durable failure after cleanup, plus all existing DIAG2 and hidden-process scenarios. DIAG3 authorizes no UAC, Live attempt, service/process/`C:\RisePals` mutation, reboot, public change, deployment, merge or RP-TURN-020 work.

## RP-TURN-019-R4-DIAG4 amendment — future non-mutating elevation probe

LIVE5's exact pre-process launcher cause remains unknown. DIAG4 introduces a closed future-only `ElevationProbe` that uses the same canonical committed PowerShell `RunAs` launch-request builder and authenticated bootstrap transport as Live, with a separate `RP-TURN-019-R4-PROBE-[A-F0-9]{8}` authorization pattern that Live cannot accept. The dedicated elevated child may validate only nonce/authorization/head/script provenance, authenticated entry, Administrator capability and a closed high/system/insufficient/unavailable integrity value before atomically writing sanitized digest-bound evidence and exiting.

The probe child cannot load or invoke the Live sequence, enumerate or mutate `C:\RisePals`, control services/SCM, create listeners, launch Node/candidate host, alter firewall/DNS/certificates/ACL/ownership, inspect `.env.local`, read credentials/data or trigger reboot/Preshutdown. Its 21 Live gates are always `not_applicable`, all Live lifecycle booleans are false and it cannot qualify as Live functional success. Checkpoint schema v4 and result schema v6 explicitly bind equivalent launch, child and optional probe diagnostics across durable pre-cleanup validation, exact cleanup and authoritative reopen. Missing, extra, tampered, inconsistent or digest-mismatched probe evidence fails closed.

DIAG4 verification is repository-only: PowerShell 5.1 AST/prohibited-operation checks, focused transport/harness regressions, the existing DIAG2/DIAG3 suites, an 11-scenario hidden separate-process boundary, `infra:test`, standard checks/build, digest/audit/client-boundary/document/security gates and zero-residue host checks. DIAG4 did not request UAC or execute ElevationProbe. A separately authorized future passing probe would prove only the elevation transport boundary, not service supervision or graceful shutdown. RP-TURN-019 remains Partial; no UAC/probe/Live attempt, host mutation, reboot, deployment, merge or RP-TURN-020 work is authorized by DIAG4.
